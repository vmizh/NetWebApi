...
public static partial class HttpUtils
{
    ...
    public static string AddQueryParam(this string url, string key, string val, bool encode = true)
    {
        ...
        if (!url.EndsWith("?") && !url.EndsWith("&")) prefix = url.IndexOf('?') == -1 ? "?" : "&";
...
#if NET6_0_OR_GREATER
    public static HttpStatusCode? GetStatus(this HttpRequestException ex)
    {
    ...
    {
        using var stream = webRes.GetResponseStream();
        using var reader = new StreamReader(stream, UseEncoding, true, 1024, true);
